...
export default function SlideViewer({ module }: Props) {
  ...
  const goNext = useCallback(() => {
    if (current >= slideCount) {
      // Already viewing the last slide — "Next" now means "finish the module,"
      // an explicit action rather than something that fires the instant the
      // counter reaches the last slide (which never gave the last slide a
    ...
    return (
      <div ref={containerRef} className="nhi-viewer-root">
        ...
        >
          ...
          {!isZoomed && (
            <>
              ...
              />
              <button
                aria-label={atEnd ? 'Finish module' : 'Next slide'}
                onClick={goNext}
                className="nhi-viewer-tapzone"
                style={{ right: 0 }}
              />
        ...
        <div className="nhi-viewer-footer">
          ...
          <button onClick={goNext} className="nhi-viewer-nav-btn">
            {atEnd ? 'Finish Module \u2713' : 'Next \u2192'}
          </button>
        </div>
